...
void IBGActionBox::Resize()
{
	...
	int lv = (IBPlannerGraph::s_iActionVarHeight + IBPlannerGraph::s_iActionVarSpace) * m_pAction->GetVariables().size();
...
void IBGActionBox::Draw() const
{
	...
	m_oCanvas.CanvasBase::Print(x+w/2, y+IBPlannerGraph::s_iActionTitleHeight/2, m_oCanvas.GetPrintFont(), IBPlannerGraph::s_iActionTitleHeight-4, Center, IBPlannerGraph::s_oActionColor, "%s", m_pAction->GetDef()->GetName().c_str());
	...
	for (IBAction::VarMap::const_iterator it = m_pAction->GetVariables().begin() ; it != m_pAction->GetVariables().end() ; ++it, ++k)
	{
		...
		m_oCanvas.CanvasBase::Print(x+w/2,
			y+IBPlannerGraph::s_iActionTitleHeight+(IBPlannerGraph::s_iActionVarHeight+IBPlannerGraph::s_iActionVarSpace)/2 + (IBPlannerGraph::s_iActionVarHeight+IBPlannerGraph::s_iActionVarSpace)*(k),
			m_oCanvas.GetPrintFont(), IBPlannerGraph::s_iActionVarHeight, RightCenter, IBPlannerGraph::s_oActionColor, "%s:", sName.c_str());

		m_oCanvas.CanvasBase::Print(x+w/2,
			y+IBPlannerGraph::s_iActionTitleHeight+(IBPlannerGraph::s_iActionVarHeight+IBPlannerGraph::s_iActionVarSpace)/2 + (IBPlannerGraph::s_iActionVarHeight+IBPlannerGraph::s_iActionVarSpace)*(k),
			m_oCanvas.GetPrintFont(), IBPlannerGraph::s_iActionVarHeight, LeftCenter, IBPlannerGraph::s_oActionColor, " %s", (pObject != NULL ? pObject->GetName().c_str() : "NULL"));
	}
}
