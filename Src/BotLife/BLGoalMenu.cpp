...
void BLGoalMenu::ConstructFromCase(const BLSquare* pSquare, const BLSquare* pSelectSquare)
{
	...
	if (pSelectSquare != NULL && pSelectSquare->GetPos() == m_oBot.GetPos())
	{
		if (pSquare->IsFree())
		{
			BLInt* dist = new BLInt(0);
			BLGoal oGoal(m_oBot.GetPlanner(), "IBFactDef_BotNearPos", &pSquare->GetPos(), dist);
			oGoal.AddObject(dist);
			m_aGoals.push_back(oGoal);
		}

		if (pSquare->GetProp() != NULL && pSquare->IsTempBlock())
		{
			m_aGoals.push_back(BLGoal(m_oBot.GetPlanner(), "IBFactDef_PropIsUnblock", pSquare->GetProp()));
		}

		if (pSquare->GetProp() != NULL && pSquare->GetProp()->IsPickable())
		{
			m_aGoals.push_back(BLGoal(m_oBot.GetPlanner(), "IBFactDef_BotHasObject", pSquare->GetProp()));
		}

		if (pSquare->IsFree() && m_oBot.GetFirstObject() != NULL)
		{
			m_aGoals.push_back(BLGoal(m_oBot.GetPlanner(), "IBFactDef_ObjectAtPos", m_oBot.GetFirstObject(), &pSquare->GetPos()));
		}

		if (pSelectSquare == pSquare && m_oBot.GetFirstObject() != NULL)
		{
			m_aGoals.push_back(BLGoal(m_oBot.GetPlanner(), "IBFactDef_BotIsEmpty"));
		}
	}
	...
	{
		m_aGoals.push_back(BLGoal(m_oBot.GetPlanner(), "IBFactDef_ObjectAtPos", pSelectSquare->GetProp(), &pSquare->GetPos()));
	}
}
...
void BLGoalMenu::UpdatePos()
{
	...
	{
		sint32 w, h;
		m_oCanva.TextSize(w, h, m_oCanva.GetPrintFont(), s_iTextHeight, "%s %s", m_aGoals[i].GetName().c_str()+10, m_aGoals[i].GetName().c_str());
...
void BLGoalMenu::Draw() const
{
	...
	for (uint i=0 ; i<m_aGoals.size() ; ++i)
	{
		...
		m_oCanva.Print(x, y, m_oCanva.GetPrintFont(), s_iTextHeight, LeftTop, c, "%s %s", m_aGoals[i].GetName().c_str()+10, m_aGoals[i].GetName().c_str());
