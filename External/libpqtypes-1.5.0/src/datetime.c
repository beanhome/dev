...
static int text2date(const char *datestr, PGdate *date, const char *datestyle);
static int bin2pgdate(int d, PGdate *date);
static int bin2pgtime(double t, int gmtoff, PGtime *time, int withtz);
static int bin2pginterval(double tval, int days, int mons, PGinterval *interval);
 ...
 *
 * timestamptz: The timestamptz is always converted to GMT.  Its adjusted by
 *   PGtime.gmtoff, so set this value to zero if you already have GMT.
 *   The following PGtimestamp members must be set: date.isbc, date.year,
 *   date.mon, date.mday, time.hour, time.min, time.sec, time.usec, time.gmtoff.
 */
static int
...
get_timestamp2(PGtypeArgs *args, int withtz)
{
	...
	if (args->format == TEXTFMT)
	{
		...
		year = BC2YEAR(pgts->date.isbc, pgts->date.year);
		date = date2j(year, pgts->date.mon+1, pgts->date.mday) - POSTGRES_EPOCH_JDATE;
...
pqt_put_interval(PGtypeArgs *args)
{
	...
	if (args->fmtinfo->integer_datetimes)
	{
		PGint8 val = (PGint8) ((((((intvl->hours * PQT_INT64CONST(60)) + intvl->mins)
			* PQT_INT64CONST(60)) + intvl->secs) * USECS_PER_SEC) + intvl->usecs);
...
{
#if defined(HAVE_STRUCT_TM_TM_ZONE) || defined(HAVE_TM_ZONE) || defined(HAVE_TZNAME)
	time_t tbuf;
	struct tm *loc;
	struct tm buf;
...
#	elif defined(HAVE_TZNAME)
	*gmtoff = -((loc->tm_isdst > 0) ? pqt_timezone - SECS_PER_HOUR : pqt_timezone);
	if (tzabbr)
...
gmtoff2name(int gmtoff, char *buf, size_t size)
{
	...
	if (secs == 0)
		...
		pqt_snprintf(buf, size, "GMT%c%02d%02d%02d", sign, gmtoff/3600, mins/60, secs);
}
