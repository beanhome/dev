...
#define DROP_TABLE(tname) PQclear(PQexec(conn, "DROP TABLE IF EXISTS " tname))
...
int main(int argc, char **argv)
{
	...
	if (!PQinitTypes(conn))
	{
		fprintf(stderr, "Failed to initialize libpqtypes as a libpq event proc\n");
		PQfinish(conn);
		return 1;
	}
  ...
  if(!PQregisterTypes(conn, PQT_COMPOSITE, &regtype, 1, 0))
	...
	else
	{
		...
		else
		{
			...
			{
				failcnt++;
				fprintf(stderr, "  PQspecPrepare - FAILED (expected %lld but got %lld)\n",
					i8+4, i8out);
			}
			else
			{
...
static void test_composite(void)
{
  ...
  PQclear(result);
	...
	result = PQexec(conn, "CREATE TYPE complex AS (t text, s simple, arr int4[][])");
	...
	/* clear all params (order doesn't matter, they are isolated structures) */
	PQparamClear(simple);
	...
	/* composite arrays come back 1 tuple per element and 1 field per attribute */
	ntups = PQntuples(comparr.res);
	if (ntups != 100)
	{
		failcnt++;
		fprintf(stderr, "put 100 elements into complex[] but got back %d\n", ntups);
		PQclear(comparr.res);
		return;
	}
	...
	for (i=0; i < ntups; i++)
	{
		...
		/* Check the simple field: reference by fname, use '#' rather than '%' */
...
static void test_varlen(int format)
{
	...
	PGnumeric num;
	PGnumeric numin = "-62731893541288039212143296120112.12431212671229121291821928918211";
	...
	result = PQexec(conn, "CREATE TABLE libpq_varlen ("
		...
		"text_a text, text_b text, bytea_a bytea, bytea_b bytea, uid uuid, n numeric)");
...
static void test_datetime(int format)
{
	...
	PGtimestamp timestampval[] = {
		{ -15565394969, {0, 1476,  9,  1, 2260432, 274, 0}, {23, 10, 31,   7214, 0, -1, 0, {0}}},   /* 1476-10-01 23:10:31.007214 */
		{-100268228408, {1, 1209,  7, 17, 1280076, 229, 1}, { 2, 19, 52,      0, 0, -1, 0, {0}}},	  /* 1209-08-17 02:19:52 BC */
		{   1198425427, {0, 2007, 11, 23, 2454458, 356, 0}, {15, 57,  7, 881542, 0, -1, 0, {0}}}};	/* 2007-12-23 15:57:07.881542 */
	PGinterval interval = {0};
	PGinterval intervalval =
		{2354, 8, 23, 14, 54, 27, 29333};  /* 2,354 years 8 mons 23 days 14:54:27.029333 */
	...
	r = PQputf(param, "%date %date %date %time %time %time %timetz %timetz %timetz "
		"%timestamp %timestamp %timestamp %interval %timestamptz",
		&dateval[0], &dateval[1], &dateval[2], &timeval[0], &timeval[1], &timeval[2],
		&timetzval[0], &timetzval[1], &timetzval[2], &timestampval[0], &timestampval[1],
		&timestampval[2], &intervalval, &timestamptzval);
	...
	r = PQgetf(result, 0, "%date %date %date %time %time %time %timetz %timetz "
	...
	else
	{
		...
		printf("    SENT: yr=%d,mon=%d,days=%d,hr=%d,min=%d,sec=%d,usec=%d\n",
			...
			intervalval.hours, intervalval.mins, intervalval.secs, intervalval.usecs);
	...
	else
	{
		...
		printf("SENT: epoch=%lld,isbc=%d,yr=%d,mon=%d,mday=%d,jday=%d,yday=%d,wday=%d,hr=%d,min=%d,sec=%d,usec=%d,withtz=%d,isdst=%d,tzabbr=%s,gmtoff=%d\n",
			...
			timestamptzval.date.isbc,timestamptzval.date.year,timestamptzval.date.mon,
			timestamptzval.date.mday,timestamptzval.date.jday,timestamptzval.date.yday,
			timestamptzval.date.wday,
			timestamptzval.time.hour, timestamptzval.time.min, timestamptzval.time.sec,
			timestamptzval.time.usec, timestamptzval.time.withtz, timestamptzval.time.isdst,
			timestamptzval.time.tzabbr, timestamptzval.time.gmtoff);

		printf("RECV: epoch=%lld,isbc=%d,yr=%d,mon=%d,mday=%d,jday=%d,yday=%d,wday=%d,hr=%d,min=%d,sec=%d,usec=%d,withtz=%d,isdst=%d,tzabbr=%s,gmtoff=%d\n",
			timestamptz.epoch,
...
static void test_network(int format)
{
	...
	if ((r = getaddrinfo("2001:4f8:3:ba:2e0:81ff:fe22:d1f1", NULL, &hints, &info)))
	{
		have_ipv6 = 0;
		//testcnt++;
		//failcnt++;
...
static void test_subclass(int format)
{
	...
	PGparam *prm;
	time_t timeout[2] = {0};
	time_t timevals[] = {0, 946702800}; /* 1970-01-01 00:00:00, 2000-01-01 00:00:00 */
	...
	result = PQparamExec(conn, prm, "INSERT INTO libpq_subclass VALUES ($1, $2)", format);
	PQparamClear(prm);
	...
	r = PQgetf(result, 0, "%libpq.epoch %epoch", 0, &timeout[0], 1, &timeout[1]);
